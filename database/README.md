...
Instructions in this table follow these common conventions: `NNN` refers to a hexadecimal memory address, `NN` refers to a hexadecimal byte, `N` refers to a hexadecimal nibble, and `X` and `Y` refer to hexadecimal register numbers (where the registers themselves are referred to as `VX` and `VY`).
